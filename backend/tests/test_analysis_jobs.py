...
def test_create_analysis_job_and_get_pending_status() -> None:
    ...
    assert result_response.json()["status"] == "pending"
...
def test_create_analysis_job_does_not_run_pipeline(
    ...
    document_id = upload_response.json()["document_id"]
    def reject_if_called(*args, **kwargs) -> None:
        raise AssertionError("pipeline must not run in the API request")
